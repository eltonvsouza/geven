...
public class BackupRestoreBean {
	...
	public void backup(){
        ...
        try{  
			...
			Process p = Runtime.getRuntime().exec("cmd /c \"" + backup + "\" -u" + getUsuario() + " -p" + getSenha() + " -x -e geven " + tabelaBackup);
    ...
    public void restore(FileUploadEvent event){
        ...
        try{  
            	Runtime.getRuntime().exec("cmd /c \"" + restore + "\" -u" + getUsuario() + " -p" + getSenha() + " qualisono < " + item.getData());
        }catch (IOException ex){  
        	ex.printStackTrace();  
        }
    }
