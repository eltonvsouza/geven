...
CREATE DATABASE IF NOT EXISTS geven;
...
('geven', '5c91eed7c7d69a7c89468828646ebfe5d917d6d51c8e58f8f7d54ab5ed8f46f4'),
('eltonvs', '5c91eed7c7d69a7c89468828646ebfe5d917d6d51c8e58f8f7d54ab5ed8f46f4');
...
CREATE TABLE IF NOT EXISTS usuario(
	...
	datahoracadasstro DATETIME DEFAULT 0,
...
-- geven/12345
(1, 'geven', '5994471abb01112afcc18159f6cc74b4f511b99806da59b3caf5a9c173cacfc5', 'Administrador do Sistema', '', 'ROLE_ADMIN'),
...
(1, 'sistema','email', 'E-Mail para o sistema enviar automaticamente','agendageven@gmail.com'),
(1, 'bancodados','nome', 'MySQL Nome do Banco','geven'),
...
CREATE TABLE IF NOT EXISTS funcionario(
  ...
  CONSTRAINT fk_funcionario_unidade FOREIGN KEY (unidade) REFERENCES unidade (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT fk_funcionario_funcionario FOREIGN KEY (filho) REFERENCES funcionario (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS contasareceber(
  ...
  parcela INT NOT NULL,
  dia INT (2) NOT NULL,
  CONSTRAINT pk_contasareceber PRIMARY KEY (codigo),
  CONSTRAINT fk_contasareceber_cliente FOREIGN KEY (cliente) REFERENCES cliente (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT fk_contasareceber_unidade FOREIGN KEY (unidade) REFERENCES unidade (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT fk_contasareceber_venda FOREIGN KEY (venda) REFERENCES venda (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE
);

-- Cria tabela PARCELAS A RECEBER
CREATE TABLE IF NOT EXISTS parcelasareceber(
  codigo INT NOT NULL AUTO_INCREMENT,
  unidade INT NOT NULL,
  contaareceber INT NOT NULL,
  datavencimento DATE NOT NULL,
  valorparcela DOUBLE (13,2) NOT NULL,
  formapagamento VARCHAR (20),
  datapagamento DATE,
  valorpago DOUBLE (13,2),
  lote BOOLEAN,
  CONSTRAINT pk_parcelasareceber PRIMARY KEY (codigo),
  CONSTRAINT fk_parcelasareceber_contasareceber FOREIGN KEY (contaareceber) REFERENCES contasareceber (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT fk_parcelasareceber_unidade FOREIGN KEY (unidade) REFERENCES unidade (codigo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE
);
